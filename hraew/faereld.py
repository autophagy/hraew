...
class LimStatistics(FaereldStatics):
    ...
    def update(self, entry):
        ...
        if 0 <= today_ordinal - entry.start.toordinal() < 365:
            ...
            if self.daily_times[days][1].get(entry.area) is None:
                self.daily_times[days][1][entry.area] = entry.end - entry.start
            else:
                self.daily_times[days][1][entry.area] += entry.end - entry.start
            self.daily_times[days][2] += entry.end - entry.start
...
class FaereldService(object):
    ...
    def get_lim_statistic(self, lim_name, statistic_name):
        if (
            lim_name in self.lim_stats
            and statistic_name.lower() in self.lim_stats[lim_name]
        ):
            return self.lim_stats.get(lim_name).get(statistic_name.lower())
